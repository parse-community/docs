...
If you have an existing app using the Parse SDK, when users upgrade to your latest app version (with data sharing enabled), the Parse SDK will automatically move the main app's local persistent data (Local Datastore, current `%{ParseUser}`, current `%{ParseInstallation}`, etc) from the app's sandbox container to the shared container.
