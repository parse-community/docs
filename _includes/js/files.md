...
`Parse.File` lets you store application files in the cloud that would otherwise be too large or cumbersome to fit into a regular `%{ParseObject}`. The most common use case is storing images, but you can also use it for documents, videos, music, and any other binary data (up to 10 megabytes).
...
Finally, after the save completes, you can associate a `Parse.File` with a `%{ParseObject}` just like any other piece of data:
