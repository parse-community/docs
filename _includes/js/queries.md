...
In many cases, `get` isn't powerful enough to specify which objects you want to retrieve. `%{ParseQuery}` offers different ways to retrieve a list of objects rather than just a single object.

The general pattern is to create a `%{ParseQuery}`, put conditions on it, and then retrieve an `Array` of matching `%{ParseObject}`s using `find`. For example, to retrieve the scores that have a particular `playerName`, use the `equalTo` method to constrain the value for a key.
